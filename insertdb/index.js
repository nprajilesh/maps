...
function getdirections(){
  ...
  //    console.log(response);
   ...
   //   console.log(polyline.getPath().getArray()[2].lat());
   //   polyline.setMap(map);
      latlngarray=polyline.getPath().getArray();
      jsondat={shape_id:1,cordinates:[]};
      for(var i=0;i<latlngarray.length;i++){
        //console.log(latlngarray[i].lat());r
        jsondat.cordinates.push({shape_pt_lng:latlngarray[i].lng(),shape_pt_lat:latlngarray[i].lat(),shape_pt_sequence:i+1});
      }
...
function inserttodb(jsondat){
    
    console.log(jsondat);
    $.ajax({
      type: 'POST',
      url: 'http://localhost:1337/shapes/create',
      data: {"shape_id":1,"route":jsondat},  ///change data variable
      dataType: 'json',
...
function load_route(){
  $.ajax({
      type: 'POST',
      url: 'http://192.168.2.10:1337/shapes/findbyid',
      data: {id:1},
      dataType: 'json',
     ...
     for(var i in jsondat.cordinates){
       var cord = new google.maps.LatLng(jsondat.cordinates[i].shape_pt_lat,jsondat.cordinates[i].shape_pt_lng);
